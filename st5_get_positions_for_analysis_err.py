...
import datetime
import csv
from lib_gz import file_db, convert_str, convert_num, data_path
...
def get_list_products_and_contracts_err():
    ...
    with sq.connect(file_db) as con:
        ...
        for row in cur.execute(
                "SELECT contract, year, find_text, customer FROM products_in_contracts WHERE in_work = 0"):
        ...
        for row in cur.execute("SELECT contract, year, find_text, customer FROM positions_err"):
...
def parse_positions_err(contract, year, customer, find_text):
    ...
    for item in position_list:
        ...
        if (name is not None) and (qtyUnit is not None) and (priceAndSum is not None):
            ...
            data.append((name, name_dop, qty, unit, price,
                         sum, contract, year, customer, find_text))
...
if __name__ == "__main__":
    ...
    list_parsing = list(get_list_products_and_contracts_err())
    ...
    while len(list_parsing) > 0:
        ...
        try:
            ...
            write_log('2. Берем в работу: ' + contract + ' / ' +
                      str(year) + ' / ' + find_text + ' / ' + customer)
            # Парсим заданную строку
            sum_items_list_parsing = len(list_parsing)
            parse_positions_err(contract, year, customer, find_text)
            list_parsing = list(get_list_products_and_contracts_err())
            ...
            write_log('1. Всего позиций для парсинга: ' +
                      str(len(list_parsing)))
            time.sleep(5)
